...
def computeMostFrequentWord(text):
    ...
    most_frequent_words = set(
        word for word, count in list(count_dict.items()) if count_dict.get(word) == max_count)  # choose words whose count is 'max_count'
...
def computeLongestPalindrome0(text):
    ...
    return 0 if len(text) == 0 else get_cost(0, len(text) - 1)
    # END_YOUR_CODE
    ...
    def pal(i, j):
        if i == j:
            ...
            if cost_matrix[i][j] is None:
                if text[i] == text[j]:
                    cost_matrix[i][j] = pal(i+1, j-1) + 2
                else:
                    cost_matrix[i][j] = max(pal(i, j-1), pal(i+1, j))
            return cost_matrix[i][j]

    return 0 if len(text) == 0 else pal(0, len(text) -1 )
